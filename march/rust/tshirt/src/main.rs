...
impl Inventory {
    ...
    fn most_stocked(&self) -> ShirtColour {
        ...
        if num_red > num_blue {ShirtColour::Red} else {ShirtColour::Blue}
    }
}
...
fn main() {
    ...
    println!("The user with pref: {:?}, is given a {:?} shirt", user_pref1, giveaway1);

    let user_pref2 = None;
    let giveaway2 = store.giveaway(user_pref2);

    println!("The user with pref: {:?}, is given a {:?} shirt", user_pref2, giveaway2);
}